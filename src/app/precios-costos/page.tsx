...
import { deletePrecioCostoPeriodo } from "@/app/actions";
...
export default async function PreciosCostosPage({
  searchParams,
}: {
  ...
  const where: Prisma.PrecioCostoMensualPeriodoWhereInput = {
  ...
  const [periodos, totalPeriodos] = await Promise.all([
    prisma.precioCostoMensualPeriodo.findMany({
      where,
    ...
    prisma.precioCostoMensualPeriodo.count({ where }),
  ]);
  ...
  const currentPath = `/precios-costos${
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-white">Precios y costos</h1>
        </div>
        <Link
          href="/precios-costos/nuevo"
      ...
      <Alert message={params?.error ?? params?.success} />
      ...
      {periodos.length === 0 ? (
        <EmptyState
          title="Sin precios y costos"
          detail="Creá un período para cargar precios promedio y costos unitarios de todos los productos."
        />
      ) : (
                ...
                <tr key={periodo.id}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <Link
                        href={`/precios-costos/${periodo.id}`}
                      ...
                      <form action={deletePrecioCostoPeriodo}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between rounded-lg border border-white/10 bg-neutral-900/75 px-4 py-3 text-sm text-neutral-300">
          ...
          <div className="flex gap-2">
            <Link
              href={`/precios-costos?${new URLSearchParams({
