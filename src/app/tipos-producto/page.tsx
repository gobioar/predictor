  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-2 text-3xl font-semibold text-white">Tipos de Producto de Venta</h1>
      </header>
      <Alert message={params?.error} />

      <form action={createTipo} className="flex flex-col gap-3 rounded-lg border border-white/10 bg-neutral-900/75 p-4 md:flex-row">
        <input
          name="nombre"
      ...
      {tipos.length === 0 ? (
        <EmptyState title="Sin tipos" detail="Creá tipos de producto para completar el maestro." />
      ) : (
              ...
              {tipos.map((tipo) => (
                <tr key={tipo.id}>
                  <td className="px-4 py-3">
                    <form id={`update-tipo-${tipo.id}`} action={updateTipo}>
                  ...
                  <td className="px-4 py-3 text-neutral-300">{tipo._count.productos}</td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <button form={`update-tipo-${tipo.id}`} className="inline-flex size-10 items-center justify-center rounded-md border border-white/10 text-neutral-200 hover:bg-white/10" title="Guardar">
                        <Save size={17} />
                      </button>
